...
Это сердце системы обработки информации POLER[n].
Способен анализировать тексты любой сложности: от заметок до полной "Одиссеи" Гомера.
Находит архетипы, паттерны, скрытые связи через резонансный анализ.
"""
...
THEMES: Dict[str, List[str]] = {
    ...
    'архетипы': [
        'герой', 'трикстер', 'мудрец', 'тень', 'анима', 'анимус',
        'мать', 'отец', 'ребенок', 'король', 'воин', 'маг', 'любовник',
        'путешествие', 'испытание', 'возвращение', 'трансформация',
        'смерть', 'возрождение', 'жертва', 'искупление', 'откровение',
        'дракон', 'змея', 'птица', 'дерево', 'вода', 'огонь', 'земля',
        'небо', 'гора', 'пещера', 'лабиринт', 'мост', 'ворота', 'ключ',
    ],
    'одиссея': [
        'Одиссей', 'Пенелопа', 'Телемах', 'Афина', 'Посейдон', 'Зевс',
        'Цирцея', 'Калипсо', 'Навсикая', 'Полифем', 'Сцилла', 'Харибда',
        'Итака', 'Троя', '斯巴达', ' Менелай', 'Елена', 'Агамемнон',
        'корабль', 'парус', 'весло', 'ветер', 'волна', 'остров',
        'возвращение', 'странствие', 'испытание', 'хитрость', 'метис',
        'гостеприимство', 'ксения', 'ностос', 'клеос', 'тиме', 'мойра',
    ],
}
...
def filter_pii(text: str) -> Tuple[str, List[Tuple[str, str]]]:
    """Фильтрация личной информации (PII)."""
...
def tokenize(text: str) -> List[str]:
    """Токенизация с удалением стоп-слов и шума."""
...
def word_rarity(word: str, total_words: int, counts: Counter) -> float:
    """Вычисляет редкость слова через энтропию."""
...
def compute_epsilon(window: TextWindow, keyword: str,
    ...
    """Вычисляет ε-энергию фрагмента.
    
    Формула: κ × intensity(keyword) × Σ(rarity²) + emotion_bonus
    
    Чем реже слова вокруг ключевого и чем больше эмоциональных маркеров —
    тем выше ε. Это указывает на семантически насыщенные участки текста.
    """
...
def compute_resonance_series(epsilons: List[float], phi_decay: float = 0.85) -> List[float]:
    """Вычисляет резонанс R[n] как затухающую сумму ε.
    
    R_t = Σ ε_i × φ^(t-i) для i ≤ t
    
    φ (phi_decay) определяет скорость затухания влияния предыдущих фрагментов.
    """
    n = len(epsilons)
    R = [0.0] * n
    for t in range(n):
...
def compute_cross_resonance(all_windows: List[TextWindow], phi_decay: float = 0.85) -> List[float]:
    """Кросс-файлный резонанс v2.0.
    
    R_t учитывает фрагменты из ВСЕХ файлов, не только текущего.
    Позволяет находить глобальные паттерны across documents.
    """
    n = len(all_windows)
    R = [0.0] * n
    for t in range(n):
...
# ═══════════════════════════════════════════════════════════════════════
# НОВОЕ v2.0: ЧТЕНИЕ ФАЙЛОВ (TXT, MD, JSON, EPUB, PNG, ARCHIVES)
# ═══════════════════════════════════════════════════════════════════════

def read_file(path: str) -> str:
    """Читает текст из файла. Поддержка: .txt, .md, .json, .epub, .zip, .tar.gz"""
    p = Path(path)
    if not p.exists():
        return ""
...
def read_archive(path: str) -> str:
    """Читает текст из архивов (ZIP, TAR, GZ) без распаковки."""
    text_parts = []
    try:
        if path.endswith('.zip'):
            with zipfile.ZipFile(path, 'r') as zf:
                for name in zf.namelist():
                    if any(name.endswith(ext) for ext in ['.txt', '.md', '.json']):
                        content = zf.read(name).decode('utf-8', errors='ignore')
                        text_parts.append(content)
        elif path.endswith('.tar.gz') or path.endswith('.tgz'):
            with tarfile.open(path, 'r:gz') as tf:
                for member in tf.getmembers():
                    if member.isfile() and any(member.name.endswith(ext) for ext in ['.txt', '.md', '.json']):
            ...
            import gzip
            with gzip.open(path, 'rt', encoding='utf-8', errors='ignore') as gf:
                text_parts.append(gf.read())
    except Exception as e:
...
def read_png_metadata(path: str) -> Dict:
    """Читает метаданные PNG (название файла как ключевое)."""
    p = Path(path)
    return {
        'filename': p.name,
...
def scan_directory(dir_path: str, extensions: List[str] = None) -> List[str]:
    ...
    if extensions is None:
        extensions = ['.md', '.txt', '.json', '.epub', '.html', '.png', '.zip', '.tar', '.gz']
    result = []
    p = Path(dir_path)
    if p.is_file():
        return [str(p)]
...
def run_poler_analyzer(
    ...
    Этапы:
    1. ℘ (Perception) — поиск вхождений ключевого слова
    2. O (Image) — создание окон контекста
    3. L (Logic) — фильтрация PII, токенизация
    4. ε (Energy) — вычисление энергии каждого фрагмента
    5. R[n] (Resonance) — вычисление резонанса
    
    Возвращает структурированный результат с топ-N фрагментами.
    """
...
class PolerAnalyzer:
    """Python API для использования poler_enhanced в других скриптах.
    
    Пример:
        from poler_enhanced import PolerAnalyzer
        
        analyzer = PolerAnalyzer(window=5000, phi=0.85, kappa=1.0, top=10)
        
        # Анализ файла
        result = analyzer.analyze_file('odyssey.md', 'Одиссей')
        
        # Анализ директории
        result = analyzer.analyze_directory('./texts', 'герой', cross_resonance=True)
        
        # Анализ текста
        result = analyzer.analyze_text('...', 'архетип')
        
        # Анализ EPUB
        result = analyzer.analyze_epub('book.epub', 'путешествие')
        
        # Сравнение версий
        diff = analyzer.diff('v1.md', 'v2.md', 'трансформация')
    """
...
def main():
    parser = argparse.ArgumentParser(
        prog='poler_enhanced',
        description=f'POLER[n] CLI v{__version__} — Сердце системы обработки информации',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Примеры:
  %(prog)s odyssey.md -k "Одиссей" --theme одиссея
  %(prog)s ./library -r -k "архетип" --cross-resonance -f md -o report.md
  %(prog)s --stdin -k "герой" < mynotes.txt
  %(prog)s v1.md v2.md --diff -k "трансформация"
  
Python API:
  from poler_enhanced import PolerAnalyzer
  analyzer = PolerAnalyzer()
  result = analyzer.analyze_file('odyssey.epub', 'путешествие')
        """
    )
    ...
    parser.add_argument('--theme', choices=list(THEMES.keys()), help='Автотема со списком ключевых слов')
    parser.add_argument('-w', '--window', type=int, default=20000, help='Размер окна (по умолчанию 20000)')
    parser.add_argument('--phi', type=float, default=0.85, help='Затухание R[n] (по умолчанию 0.85)')
    parser.add_argument('--kappa', type=float, default=1.0, help='Интенсивность ε (по умолчанию 1.0)')
    parser.add_argument('--top', type=int, default=10, help='Топ-N окон (по умолчанию 10)')
    ...
    if args.recursive and args.input:
        ...
        if fmt == 'json':
            ...
            for r in all_results:
                top_eps = _fmt(r["top_by_epsilon"][0]["epsilon"], 0) if r["top_by_epsilon"] else "0"
