#!/usr/bin/env python3
"""LLM runner — dispatches to z-ai chat subcommand."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    if not query:
        print(json.dumps({"skill": "LLM", "status": "error",
                          "error": "no query provided"}, ensure_ascii=False, indent=2))
        sys.exit(1)

    cmd = [Z_AI, "chat", "--prompt", query]
    try:
    ...
    if r.returncode != 0:
        ...
        try:
            ...
            result = run_skill("LLM", user_query=query)
        ...
        try:
            ...
            if content:
                brief = {
                    ...
                    "claims": [{"text": content, "source": "LLM:z-ai-chat",
                                "confidence": 0.85}],
                    "metadata": {"method": "z-ai-chat", "raw_env": env.get("id", "")},
                }
    ...
    brief = {
        ...
        "claims": [{"text": out, "source": "LLM:z-ai-chat",
                     "confidence": 0.5}],
        "metadata": {"method": "z-ai-chat", "raw": True},
    }
    print(json.dumps(brief, ensure_ascii=False, indent=2))
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    result = run_skill("LLM", user_query=args.query, backend=args.backend)
    sys.exit(0 if result.get("status") == "success" else 1)
