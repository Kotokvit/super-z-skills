#!/usr/bin/env python3
"""VLM runner — dispatches to z-ai vision subcommand."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    if not query:
        print(json.dumps({"skill": "VLM", "status": "error",
    ...
    if r.returncode != 0:
        ...
        try:
            ...
            result = run_skill("VLM", user_query=query)
        ...
        try:
            ...
            if content:
                brief = {
                    ...
                    "claims": [{"text": content, "source": "VLM:z-ai-vision",
                                "confidence": 0.85}],
    ...
    brief = {
        ...
        "claims": [{"text": out, "source": "VLM:z-ai-vision",
                     "confidence": 0.5}],
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    result = run_skill("VLM", user_query=args.query, backend=args.backend)
    sys.exit(0 if result.get("status") == "success" else 1)
