#!/usr/bin/env python3
"""ASR runner — dispatches to z-ai asr subcommand."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    if not query:
        print(json.dumps({"skill": "ASR", "status": "error",
                          "error": "no query provided"}, ensure_ascii=False, indent=2))
        sys.exit(1)

    cmd = [Z_AI, "asr", "--prompt", query]
    try:
    ...
    if r.returncode != 0:
        ...
        try:
            ...
            result = run_skill("ASR", user_query=query)
        ...
        try:
            ...
            if content:
                brief = {
                    ...
                    "claims": [{"text": content, "source": "ASR:z-ai-asr",
                                "confidence": 0.85}],
                    "metadata": {"method": "z-ai-asr", "raw_env": env.get("id", "")},
                }
    ...
    brief = {
        ...
        "claims": [{"text": out, "source": "ASR:z-ai-asr",
                     "confidence": 0.5}],
        "metadata": {"method": "z-ai-asr", "raw": True},
    }
    print(json.dumps(brief, ensure_ascii=False, indent=2))
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    result = run_skill("ASR", user_query=args.query, backend=args.backend)
    sys.exit(0 if result.get("status") == "success" else 1)
