#!/usr/bin/env python3
"""image-search runner — dispatches to z-ai image-search subcommand."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    if not query:
        print(json.dumps({"skill": "image-search", "status": "error",
                          "error": "no query provided"}, ensure_ascii=False, indent=2))
        sys.exit(1)

    cmd = [Z_AI, "image-search", "--prompt", query]
    try:
    ...
    if r.returncode != 0:
        ...
        try:
            ...
            result = run_skill("image-search", user_query=query)
        ...
        try:
            ...
            if content:
                brief = {
                    ...
                    "claims": [{"text": content, "source": "image-search:z-ai-image-search",
                                "confidence": 0.85}],
                    "metadata": {"method": "z-ai-image-search", "raw_env": env.get("id", "")},
                }
    ...
    brief = {
        ...
        "claims": [{"text": out, "source": "image-search:z-ai-image-search",
                     "confidence": 0.5}],
        "metadata": {"method": "z-ai-image-search", "raw": True},
    }
    print(json.dumps(brief, ensure_ascii=False, indent=2))
