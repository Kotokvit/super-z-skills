#!/usr/bin/env python3
"""image-understand runner — dispatches to z-ai vision subcommand."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    if not query:
        print(json.dumps({"skill": "image-understand", "status": "error",
                          "error": "no query provided"}, ensure_ascii=False, indent=2))
        sys.exit(1)

    cmd = [Z_AI, "vision", "--prompt", query]
    try:
    ...
    if r.returncode != 0:
        ...
        try:
            ...
            result = run_skill("image-understand", user_query=query)
        ...
        try:
            ...
            if content:
                brief = {
                    ...
                    "claims": [{"text": content, "source": "image-understand:z-ai-vision",
                                "confidence": 0.85}],
                    "metadata": {"method": "z-ai-vision", "raw_env": env.get("id", "")},
                }
    ...
    brief = {
        ...
        "claims": [{"text": out, "source": "image-understand:z-ai-vision",
                     "confidence": 0.5}],
        "metadata": {"method": "z-ai-vision", "raw": True},
    }
    print(json.dumps(brief, ensure_ascii=False, indent=2))
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    result = run_skill("image-understand", user_query=args.query, backend=args.backend)
    sys.exit(0 if result.get("status") == "success" else 1)
